...
app.get('/config/oep-mocked.js', function(req, res, next) {
  getFirebaseConfig().then(function(config){
    res.set('Content-Type', 'application/javascript');
    res.send('(' + oepMocked.module.toString() + ')(angular, ' + JSON.stringify(config.url) + ');');
  }).catch(next);
});


app.get('/config/oep-e2e-mocked.js', function(req, res, next) {
  getFirebaseConfig().then(function(config){
    res.set('Content-Type', 'application/javascript');
    res.send('(' + oepE2EMocked.module.toString() + ')(angular, ' + JSON.stringify(config) + ');');
  }).catch(next);
});
